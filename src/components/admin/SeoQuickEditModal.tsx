  ...
  import { seoQuickEditSchema, SeoQuickEditFormData } from "@/lib/schemas/seo-validation";
  import { toast } from "@/components/ui/toast";
  import { Loader2 } from "lucide-react";

  interface SeoQuickEditModalProps {
    pageId: string;
    isOpen: boolean;
    onClose: () => void;
    onSaved?: () => void;
  ...
  export function SeoQuickEditModal({
    ...
    const {
      ...
      setError,
      formState: { errors },
    } = useForm<SeoQuickEditFormData>({
      resolver: zodResolver(seoQuickEditSchema) as any,
      defaultValues: {
    ...
    const onSubmit: SubmitHandler<SeoQuickEditFormData> = async (data) => {
      setIsSaving(true);
      try {
